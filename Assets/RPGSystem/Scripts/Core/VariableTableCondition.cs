    ...
    public class VariableTableCondition
    {
        ...
        public void SubscribeToConditionTable(List<int> _subscribedSwitchList, List<int> _subscribedVariableList, List<int> _subscribedLocalVariableList, Action action)
        {
            foreach (var s in switchTable)
            {
                ...
                RPGManager.Instance.gameState.SubscribeToSwitchChangedEvent(ID, action);
            ...
            foreach (var v in variableTable)
            {
                ...
                RPGManager.Instance.gameState.SubscribeToVariableChangedEvent(ID, action);
            ...
            foreach (var lv in localVariableTable)
            {
                ...
                if (!_subscribedLocalVariableList.Contains(ID))
                {
                    ...
                    RPGManager.Instance.gameState.SubscribeToLocalVariableChangedEvent(ID, action);
                }
            }
        }
        ...
        {
            foreach (var id in _subscribedLocalVariableList) RPGManager.Instance.gameState.UnsubscribeToLocalVariableChangedEvent(id, action);
            foreach (var id in _subscribedSwitchList) RPGManager.Instance.gameState.UnsubscribeToSwitchChangedEvent(id, action);
            foreach (var id in _subscribedVariableList) RPGManager.Instance.gameState.UnsubscribeToVariableChangedEvent(id, action);
        ...
        public bool IsAllConditionOK()
        {
            ...
            {
                var variableValue = RPGManager.Instance.gameState.GetLocalVariable(requiredLocalVariable.ID());
            ...
            {
                var switchValue = RPGManager.Instance.gameState.GetSwitch(requiredSwitch.ID());
                if (requiredSwitch.value != switchValue) return false;
            }
            foreach (var requiredVariable in variableTable)
            {
                var variableValue = RPGManager.Instance.gameState.GetVariable(requiredVariable.ID());
